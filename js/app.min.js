...
if (profileHeader) {
    ...
    profileMedia.addListener(profileDel);
    profileDel(profileMedia);
}
...
if (document.querySelector('.slider-reasons__body')) {
    ...
    function addNextPrevious(itemActive) {
        line.style.transform = `rotate(${itemActive.dataset.rotate}deg)`;
        const itemActiveNextElement = itemActive.nextElementSibling;
        const itemActivePreviousElement = itemActive.previousElementSibling;
        if (itemActiveNextElement) {
            itemActiveNextElement.classList.add('_next')
        }
        if (itemActivePreviousElement) {
            itemActivePreviousElement.classList.add('_prev')
        }
    }
    addNextPrevious(itemActive);
    ...
    function removeClasses() {
        ...
        const itemPrev = chartReasons.querySelector('.chart-reasons__item._prev');
        itemActive.classList.remove('_active')
        if (itemNext) {
            itemNext.classList.remove('_next')
        }
        if (itemPrev) {
            itemPrev.classList.remove('_prev')
        }
    }
    ...
    function changeActive(item) {
        ...
        addNextPrevious(item);
    }

    document.addEventListener("click", function (e) {
        const target = e.target;
        if (target.closest('.chart-reasons__item-number')) {
